...
def display_inventory():
    ...
    if 'зелье удачи' in player['inventory']:
        option = input('''Желаешь ли съесть шаверму?
1 - да
2 - нет''')
        if option == '1':
            player["luck"] += 7
            print(f'теперь ваш шанс нанести критический урон равен {player["luck"]}%')
            player['inventory'].remove('Шаверма')
...
def shop():
    ...
    item = input('Что вы выберите: ')
    if item in player['inventory']:
        print('у тебя уже есть такой предмет')
    elif player['money'] >= items[item]['price']:
        print(F'Ты успешно прирбрел {items[item]["name"]}')
        player['inventory'].append(items[item]["name"])
        player['money'] -= items[item]['price']

def earn():
    print('Добро пожаловать в полицейский участок! У тебя есть 66% шанс заработать 500 монет.')
